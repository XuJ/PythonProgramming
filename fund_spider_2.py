...
def randHeader():
    ...
    head_user_agent = ['Opera/8.0 (Macintosh; PPC Mac OS X; U; en)', 'Opera/9.27 (Windows NT 5.2; U; zh-cn)',
...
class FundSpiders():
    ...
    def getFundInfo(self, fund_code):
        ...
        try:
            mySQL.insertData('fund_info',
                             result)  # print (self.getCurrentTime(),'Fund Info Insert Sucess:', result[  #
            # 'fund_code'],result['fund_name'],result['fund_abbr_name'],result['fund_manager'],result['funder'],
            #  result['establish_date'],result['establish_scale'],result['benchmark'] )
        except  Exception as e:
            print(self.getCurrentTime(), fund_code, fund_url, e)
        ...
        try:
            print(self.getCurrentTime(), 'getFundInfo:', result['fund_code'], result['fund_name'],
                  ...
                  # ,result['issue_date'],result['asset_value'],result['asset_value_date'], result['unit'],
                  # result['unit_date'],result['fund_trustee']
                  # ,result['total_div'],result['mg_fee'],result['trust_fee'], result['sale_fee'], result['buy_fee'],
                  # result['buy_fee2'],result['underlying']
                  )
        except  Exception as e:
            print(self.getCurrentTime(), fund_code, fund_url, e)
                ...
                for a in tr.findAll('a'):
                    if a:
                        try:
                            ...
                            manager[
                                'data_source'] = 'eastmoney'  # print (self.getCurrentTime(),manager['manager_id'],
                            #  manager['manager_name'],manager['url'])
                        except Exception as e:
    ...
    def getFundNav(self, fund_code):
        ...
        try:
           ...
           '''
            ...
            res = getURL(fund_url)
            # 获取历史净值的总记录数
            records = (res.text.strip('var apidata=').strip('{;}').split(',')[1].strip(
                'records:'))  # print(  # res.text.strip('var apidata=').strip('{;}').split(','))  # print (records)
        except  Exception as e:
            print(self.getCurrentTime(), 'getFundNav1', fund_code, fund_url, e)
        try:
            # 根据基金代码和总记录数，一次返回所有历史净值
            fund_nav = 'http://fund.eastmoney.com/f10/F10DataApi.aspx?type=lsjz&code=' + fund_code + '&page=1&per=' +\
                       records
            res = getURL(fund_nav)
            soup = BeautifulSoup(res.text, 'html.parser')
        except  Exception as e:
        ...
        for tr in tab.findAll('tr'):
            ...
            if tr.findAll('td') and len((tr.findAll('td'))) == 7:
                ...
                try:
                    ...
                    result['div_record'] = tr.select('td:nth-of-type(7)')[0].getText().strip().strip(
                        '\'')  # print (  # self.getCurrentTime(),i,result['fund_code'],result['the_date'],
                    # result['nav'],  # result['add_nav'],result['nav_chg_rate'],result['buy_state'],
                    # result['sell_state'] )
                except  Exception as e:
            ...
            # 如果是货币基金，获取万份收益和7日年化利率
                ...
                try:
                    ...
                    result['div_record'] = (tr.select('td:nth-of-type(6)')[
                        0].getText().strip())  # print (  # self.getCurrentTime(),i,result['fund_code'],
                    # result['the_date'],result['nav'],  # result['add_nav'],result['nav_chg_rate'],
                    # result['buy_state'],result['sell_state'] )
                except  Exception as e:
